...
def test_list_saved_unauthorized(client: TestClient):
    """Test GET /api/v1/saved-properties requires auth."""
    response = client.get("/api/v1/saved-properties")
    assert response.status_code in [401, 403]


def test_list_saved_authorized(client: TestClient, user_access_token: str):
...
def test_save_property_invalid_uuid(client: TestClient, user_access_token: str):
